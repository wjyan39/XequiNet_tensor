...
def set_init_attr(dataset: Dataset, config: NetConfig, **kwargs):
    ...
    dataset.transform = kwargs.get("transform", None)
    dataset.pre_transform = kwargs.get("pre_transform", None)
    ...
    dataset._virtual_dim = False if config.reduce_op is None else True
...
def process_pbch5(f_h5: h5py.File, mode: str, cutoff: float, prop_dict: dict, **kwargs):
    ...
    max_edges = kwargs.get("max_edges", 100)
    virtual_dim = kwargs.get("virtual_dim", True)
    # triplet = kwargs.get("triplet", False)
    # loop over samples
    for pbc_name in f_h5[mode].keys():
        ...
        for icfm, coord in enumerate(coords):
            ...
            ltc = torch.from_numpy(lattice).to(torch.get_default_dtype()).unsqueeze(0)
            data = Data(at_no=at_no, pos=coord, edge_index=edge_index, shifts=shifts, charge=charge, spin=spin, lattice=ltc)
            # if triplet:
            #     data.triplet_index = edge2triplet(edge_index, at_no.size(0))
            for p_attr, p_name in prop_dict.items():
