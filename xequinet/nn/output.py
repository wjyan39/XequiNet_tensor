...
class CartTensorOut(nn.Module):
    ...
    def __init__(
        ...
        if output_dim == 3**(order) or self.trace_out:
            ...
            rtp = o3.ReducedTensorProducts(formula=required_symmetry, filter_ir_out=filter_ir_out, **vec_space)
        else:
...
class CSCOut(nn.Module):
    ...
    def __init__(
        ...
        vec_space = vector_space if vector_space is not None else {i: "1o" for i in self.indices}
        self.rtp = o3.ReducedTensorProducts(formula=required_symmetry, **vec_space) 
        self.irreps_out = self.rtp.irreps_out 
        self.activation = resolve_actfn(actfn)
        # tensor product path 
        irreps_tp_out, instructions = get_feasible_tp(
...
class GeneralOut(nn.Module):
    """
    Output Module for general Cartesian order-N tensor as well as scalar target. 
    """
    def __init__(
        self,
        node_dim: int = 128,
        edge_irreps: Union[str, o3.Irreps, Iterable] = "128x0e + 64x1o + 32x2e",
        hidden_dim: int = 64,
        hidden_irreps: Union[str, o3.Irreps, Iterable] = "32x0e + 32x1o + 32x2e",
        order: int = 2,
        required_symmetry: str = "ij",
        target_irreps: Optional[Union[str, o3.Irreps, Iterable]] = None,
        output_dim: int = 9,
        actfn: str = "silu",
    ):
        super().__init__()
        ...
        self.indices = required_symmetry.split("=")[0].replace("-", "") 
        lmin = min(self.irreps_in.lmax, self.irreps_hidden.lmax)
        assert lmin * 2 >= self.order, f"Input irreps must have 2*lmax >= order, got lmax={lmin} and order={order}."
        if self.order == 0 or self.output_dim == 1:
            self.irreps_out = o3.Irreps(f"{self.output_dim}x0e") 
        else:
            if target_irreps is not None:
                target_irreps = target_irreps if isinstance(target_irreps, o3.Irreps) else o3.Irreps(target_irreps)
            self.rtp = o3.ReducedTensorProducts(formula=required_symmetry, filter_ir_out=target_irreps, **{i: "1o" for i in self.indices}) 
            self.irreps_out = self.rtp.irreps_out 
        self.activation = resolve_actfn(actfn)
        # tensor product
        irreps_tp_out, instructions = get_feasible_tp(
    ...
    def forward(
        ...
        Args:
            ...
            `x_spherical`: Spherical features.
        Returns:
            `res`: order-N Cartesian `Tensor` output.
        """
        batch = data.batch 
        x_sph = self.lin_out_pre(x_spherical)
        tp_weights = self.gate_mlp(x_scalar)
        x_tp = self.tp(x_sph, x_sph, tp_weights) 
        atom_sph = self.lin_out_post(x_tp) if self.post_trans else x_tp
        res_sph = scatter(atom_sph, batch, dim=0)
        if self.order == 0 or self.output_dim == 1:
            res = res_sph
        else:
            Q = self.rtp.change_of_basis
            ...
            # permute (y, z, x) to (x, y, z)
            for i_dim in range(1, len(self.indices)+1):
                res = torch.index_select(res_cart, dim=-i_dim, index=self.cartesian_index)
        
        return res


class CartTensorTP(nn.Module):
    """
    Output Module for order n tensor via tensor product of input hidden irreps and cartesian transform. 
    """
    def __init__(
        ...
        super().__init__()
        self.irreps_in = edge_irreps if isinstance(edge_irreps, o3.Irreps) else o3.Irreps(edge_irreps)
        lmax_in = self.irreps_in.lmax 
        assert max_l <= 2*lmax_in, f"The input irreps cannot generate l={max_l} channels via tensor product. Got lmax_in={lmax_in}."
        assert max_l * 2 >= order, f"Input irreps must have 2*lmax >= order, got lmax={max_l} and order={order}."
        # self.irreps_hidden = o3.Irreps([(hidden_irrep_dim, (l, (-1)**l)) for l in range(lmax_in + 1)])  
        self.irreps_hidden = o3.Irreps([(hidden_irrep_dim, (l, p)) for _, (l, p) in self.irreps_in])
        self.trace_out = output_dim == 1 and order == 2 
        self.indices = required_symmetry.split("=")[0].replace("-", "")
        vec_space = vector_space if vector_space is not None else {i: "1o" for i in self.indices}
        # determine various metadata
        ...
        irreps_hidden_base = [(hidden_irrep_dim, (0, 1))]
        for l in range(1, max_l+1):
            irreps_hidden_base.append((hidden_irrep_dim, (l, -1)))
            irreps_hidden_base.append((hidden_irrep_dim, (l,  1))) 
        # irreps_hidden_base.append((hidden_irrep_dim, (2*lmax, 1)))
        self.irreps_hidden_base = o3.Irreps(irreps_hidden_base)
        # expansion to include parity
        irreps_mix_out, instruct_mix = get_feasible_tp(
            self.irreps_hidden, self.irreps_hidden, self.irreps_hidden_base, "uuu",
        )
        self.tp_mix = o3.TensorProduct(
            self.irreps_hidden,
            self.irreps_hidden,
            irreps_mix_out,
            instruct_mix,
        ...
        self.o3norm = resolve_o3norm("layer", irreps_mix_out)

        # tensor product path
        irreps_tp_out, instruct_out = get_feasible_tp(
            irreps_mix_out, irreps_mix_out, self.irreps_out, "uuw",
        )
        self.tp_out = o3.TensorProduct(
            irreps_mix_out,
            irreps_mix_out,
            irreps_tp_out,
            instructions=instruct_out,
            internal_weights=False,
            shared_weights=False,
        )
        ...
        self.gate_mlp = nn.Sequential(
            nn.Linear(node_dim, hidden_dim, bias=True),
            self.activation,
            nn.Linear(hidden_dim, self.tp_out.weight_numel, bias=True)
        )
        if irreps_tp_out == self.irreps_out:
            self.post_trans = False 
        else:
    ...
    def forward(
        ...
        x_mix = self.tp_mix(x_sph_U, x_sph_V)
        x_mix = self.o3norm(x_mix)

        # x_mix_U = self.lin_out2_U(x_mix)
        # x_mix_V = self.lin_out2_V(x_mix)
        tp_weights = self.gate_mlp(x_scalar)
        x_tp = self.tp_out(x_mix, x_mix, tp_weights) 
        # x_tp = self.tp_out(x_mix_U, x_mix_V, tp_weights)
        atom_out = self.lin_out_post(x_tp) if self.post_trans else x_tp
        if self.reduce_op is not None:
            res_sph = scatter(atom_out, batch, dim=0, reduce=self.reduce_op)
        else:
        ...
        if self.trace_out:
            res = torch.diagonal(res_cart, dim1=-2, dim2=-1).sum(dim=-1, keepdim=True) / 3.0
            return res
        else:
            # permute (y, z, x) to (x, y, z)
            for i_dim in range(1, len(self.indices)+1):
